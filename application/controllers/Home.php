...
class Home extends CI_Controller 
{
	...
	{
        //$data['all_settings']=$this->home_model->get_all_service_data();
        /*echo "<pre>";
        print_r($data['all_settings']);
        echo "</pre>";
        exit();*/
        $data['event_settings']=$this->home_model->get_all_events_data();
        $data['service_settings']=$this->admin_model->get_all_service_setting_data();
        $data['testemonial_setting']=$this->home_model->get_all_testemonials();
        $data['banner_data']=$this->home_model->fetch_banner_data_with_status_active();
   ...
   private function get_my_single_array_from_multi_array($multi_data_array,$index_name,$index_value,$index_add_info)
   {
        $my_data_array=array();
        if(!empty($multi_data_array)):
            foreach($multi_data_array as $d):
                $myindex=$d[$index_name];       //geting content_type in the $index_name 
                $myvalue=$d[$index_value];      //geting content_description in the $index_value 
                $myaddinfo=$d[$index_add_info];    //getting additional info in the index $$index_add_info
                $my_arr=array($myindex=>array('value'=>$myvalue,'info'=>$myaddinfo)); // setting the $myindex as the key where as the $myvalue as the value of the key $myindex
                $my_data_array=array_merge($my_data_array,$my_arr);    // merging the array that we are getting
            endforeach;
        endif;
